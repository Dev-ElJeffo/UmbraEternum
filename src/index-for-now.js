...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    console.log(`[${new Date().toISOString()}] [HTTP] ${req.method} ${req.originalUrl} - STATUS: ${res.statusCode} - ${duration}ms`);
  });
...
io.on('connection', (socket) => {
  ...
  console.log(`[${new Date().toISOString()}] [SOCKET] Nova conexão: ${socket.id} - IP: ${clientIP}`);
  ...
  socket.onAny((event, ...args) => {
    if (event !== 'ping' && event !== 'pong') {
      console.log(`[${new Date().toISOString()}] [SOCKET] Evento recebido de ${socket.id}: ${event}`);
    }
  });
  ...
  socket.on('authenticate', (data) => {
    ...
    console.log(`[${new Date().toISOString()}] [SOCKET] Dados recebidos:`, typeof data, data ? (typeof data === 'object' ? 'Objeto' : data.substring(0, 20) + '...') : 'null');
    ...
    try {
      ...
      if (typeof data === 'object' && data !== null) {
        ...
        console.log(`[${new Date().toISOString()}] [SOCKET] Token extraído do objeto:`, tokenString ? tokenString.substring(0, 20) + '...' : 'null');
      }
      
      // Verificar o token JWT
      const user = verifyToken(tokenString);
      console.log(`[${new Date().toISOString()}] [SOCKET] Resultado da verificação do token:`, user ? `Usuário autenticado: ${user.username}` : 'Falha na autenticação');
      ...
      if (user) {
        ...
        for (const [existingSocketId, existingUser] of authenticatedSockets.entries()) {
          if (existingUser.id === user.id) {
            console.log(`[${new Date().toISOString()}] [SOCKET] Usuário ${user.username} já está conectado em outro socket (${existingSocketId})`);
        ...
        console.log(`[${new Date().toISOString()}] [SOCKET] Usuário ${user.username} autenticado com sucesso`);
        ...
        console.log(`[${new Date().toISOString()}] [SOCKET] Falha na autenticação para socket ${socket.id}`);
  ...
  socket.on('disconnect', (reason) => {
    console.log(`[${new Date().toISOString()}] [SOCKET] Desconexão: ${socket.id} - Motivo: ${reason}`);
...
app.put('/api/admin/users/:id/ban', authenticate, isAdmin, async (req, res) => {
  try {
    ...
    // Verificar se o usuário existe
    const [users] = await dbPool.query('SELECT id, username, role FROM users WHERE id = ?', [userId]);
...
app.put('/api/admin/users/:id/promote', authenticate, isAdmin, async (req, res) => {
  try {
    ...
    // Log da ação
    logActivity(`Administrador ${req.username} promoveu o usuário ${user.username} a administrador`, 'admin');
...
app.get('/api/admin/stats', authenticate, isAdmin, async (req, res) => {
  try {
    ...
    // Personagens ativos
    const [activeCharacters] = await dbPool.query('SELECT COUNT(*) as count FROM characters WHERE active = TRUE');
  ...
  try {
    ...
    for (const [socketId, user] of authenticatedSockets.entries()) {
      if (user.id === userId) {
        const socket = io.sockets.sockets.get(socketId);
        if (socket) {
          console.log(`[${new Date().toISOString()}] [SOCKET] Desconectando usuário ${username} no socket ${socketId}`);
          socket.disconnect(true);
