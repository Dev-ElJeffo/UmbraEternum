...
class RefreshTokenModel {
  ...
  async create(userId: number, expiresIn: string = '7d'): Promise<RefreshToken> {
    try {
      ...
      const [tokens]: any = await pool.query('SELECT * FROM refresh_tokens WHERE id = ?', [
        tokenId,
      ]);
  ...
  async findByToken(token: string): Promise<RefreshToken | null> {
    try {
      const [tokens]: any = await pool.query('SELECT * FROM refresh_tokens WHERE token = ?', [
        token,
      ]);
  ...
  async delete(token: string): Promise<boolean> {
    try {
      const [result]: any = await pool.query('DELETE FROM refresh_tokens WHERE token = ?', [token]);
  ...
  async deleteAllForUser(userId: number): Promise<boolean> {
    try {
      const [result]: any = await pool.query('DELETE FROM refresh_tokens WHERE user_id = ?', [
        userId,
      ]);
