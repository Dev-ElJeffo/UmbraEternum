...
class CharacterModel {
  ...
  async create(characterData: Character): Promise<Character> {
    try {
      ...
      const [characters]: any = await pool.query('SELECT * FROM characters WHERE id = ?', [
        characterId,
      ]);
  ...
  async findById(id: number): Promise<Character | null> {
    try {
      const [characters]: any = await pool.query('SELECT * FROM characters WHERE id = ?', [id]);
  ...
  async findByUserId(userId: number): Promise<Character[]> {
    try {
      const [characters]: any = await pool.query('SELECT * FROM characters WHERE user_id = ?', [
        userId,
      ]);
  ...
  async updateLastActive(id: number): Promise<void> {
    try {
      await pool.query('UPDATE characters SET last_active = CURRENT_TIMESTAMP WHERE id = ?', [id]);
    } catch (error) {
  ...
  async delete(id: number): Promise<boolean> {
    try {
      const [result]: any = await pool.query('DELETE FROM characters WHERE id = ?', [id]);
