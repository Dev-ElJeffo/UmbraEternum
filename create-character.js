...
// Criar personagem
async function createCharacter(token, characterData) {
...
async function main() {
  ...
  // Listar personagens antes da criação
  console.log('\n=== Personagens antes da criação ===');
  await listCharacters(adminToken);
  
  // Criar um personagem para o admin
  ...
  const characterData = {
    ...
    intelligence: 18,
    dexterity: 12,
    appearance: 'Um mago de cabelos prateados e olhos azuis brilhantes.'
  };
  
  const createResult = await createCharacter(adminToken, characterData);
  
  if (createResult.success) {
    console.log('Personagem criado com sucesso!');
    
    // Listar personagens após a criação
    console.log('\n=== Personagens após a criação ===');
    await listCharacters(adminToken);
  } else {
    console.error('Falha ao criar personagem.');
  }
}
